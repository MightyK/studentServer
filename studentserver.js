    ...
    if (checkStudentExists() == false) {
      fs.writeFile("students/" + record_id + ".json", str, function (err) {//writes to the students directory
        ...
        if (err) {
          ...
          rsp_obj.first_name = obj.first_name;
          rsp_obj.last_name = obj.last_name;
          rsp_obj.gpa = obj.gpa;
          rsp_obj.enrolled = obj.enrolled;
 ...
 *     description: Use this endpoint to retrieve student(s) based on their last name.
 *     parameters:
 *       - name: last_name
 *         description: Student's last name
 *         in: path
 ...
 *     responses:
 *       200:
 *         description: Success. The student object(s) has been retrieved.
 *       404:
 *         description: Error. The requested resource was not found.
 */
app.get('/students/search/:lastname', function (req, res) {
  ...
  glob("students/*.json", null, function (err, files) {
    ...
    readStudents(files, arr, res, last_name);
